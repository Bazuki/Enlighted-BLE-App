...
class WatchDevice: NSObject, NSCoding, ObservableObject
{
    ...
    var connectedMimicDevices = [WatchDevice]();
    ...
    // MARK: Singleton
        ...
        // initializing as nil
    public static var connectedDevice = WatchDevice(isFakeDevice: true, equalsNil: true);
    ...
    public static func setConnectedDevice(newDevice: WatchDevice)
    {
        connectedDevice = newDevice;
    }
    
    public static func createDemoDevice() -> WatchDevice
    {
        let demoDeviceData = NSDataAsset.init(name: "Data");
        var devices = NSKeyedUnarchiver.unarchiveObject(with: demoDeviceData!.data) as? [WatchDevice];
...
//            }
        ...
        if (WatchDevice.profiling && WatchDevice.currentlyProfiling)
        {
            ...
            WatchDevice.lastTxTimestamp = Date().timeIntervalSince(WatchDevice.profilerStopwatch);
            commandString += (inputInts.map { String($0) }.joined(separator: " "));
            let newMainLine = "\(commandString),\(Date().timeIntervalSince(WatchDevice.profilerStopwatch)),\(3),\(duration),\n";
            let newTxLine = "\(commandString),\(duration)\n";
            WatchDevice.mainCsvText.append(contentsOf: newMainLine);
            WatchDevice.txCsvText.append(contentsOf: newTxLine);
    ...
    public static func reportWatchError(_ error: ENL_ERROR, additionalInfo: String = "")
    {
        
        if (Constants.SEND_EMAIL_ERROR_REPORTS && error.promptPopup)
        {
            let emailContents = [0: "<p>Received an error with error code \(error.errorCode) named \(error.name). </p>  <p>\(additionalInfo)</p>    <p>Timestamp: \(formatFilenameTimestamp(fileTimeStamp))</p>   <p> Software version: \(( Bundle.main.infoDictionary?["CFBundleShortVersionString"] as! String?) ?? "unknown") </p> <p> On Apple Watch </p> <p>     Hardware name: \(WatchDevice.connectedDevice!.name) \(WatchDevice.connectedDevice!.nickname) </p><p>     Hardware type: \(WatchDevice.connectedDevice!.hardwareVersion) </p> ", 1: "\(error.errorCode)"];
